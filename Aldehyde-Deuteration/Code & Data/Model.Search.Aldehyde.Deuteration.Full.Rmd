---
title: "Aldehyde Deuteration - Model Search - full dataset"
output:
  word_document:
    highlight: null
editor_options:
...
``` {r Model Search}
data <- data.table::fread('LEC_Data.csv', header = T) # Import data
...
models <- sub_model_ordinal(data = kfold.data,
                                               min = 4,
                                               max = 4)
  
#models <- data.frame(data.table::fread('4_features_models_ordinal_full_data.csv'))[, -1]
 
knitr::kable(models) # present list of models
write.csv(models, '4_features_models_ordinal_full_data.csv')

test.data <- kfold.data
