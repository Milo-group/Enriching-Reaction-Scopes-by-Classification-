---
title: "Aldehyde Deuteration - Best Model Feature Analysis"
output:
  word_document:
    highlight: null
editor_options:
...
models <- sub_model_ordinal(data = kfold.data[uni_similarties, ],
                                               min = 4,
                                               max = 4)
  
#models <- data.frame(data.table::fread('4_features_models_ordinal.csv'))[, -1]
 
knitr::kable(models) # present list of models
#write.csv(models, '4_features_models_ordinal.csv')
...
test <- MASS::polr(test.form,
                   data = test.data,
                   Hess = T, control = list(maxit = 100), start = start)
...
mod.info.ordinal(test, Predictions_data, F)
...
test.form <- data.frame("`class` ~ `-2-3-` + `dip_y` + `Dist(2, 7)`")

names(test.form) <- 'formula'

knitr::kable(test.form)

test.form <- "`class` ~ `-2-3-` + `dip_y` + `Dist(2, 7)`"
...
ct.plot(class.table, 
        plot.title = 'NPA_1 Removed',
        conformation = 'External Validation')
...
test.form <- data.frame("`class` ~ `-2-3-` + `dip_y` + `NPA_1`")

names(test.form) <- 'formula'

knitr::kable(test.form)

test.form <- "`class` ~ `-2-3-` + `dip_y` + `NPA_1`"
...
test.form <- data.frame("`class` ~ `-2-3-` + `Dist(2, 7)` + `NPA_1`")

names(test.form) <- 'formula'

knitr::kable(test.form)

test.form <- "`class` ~ `-2-3-` + `Dist(2, 7)` + `NPA_1`"
...
test.form <- data.frame("`class` ~ `dip_y` + `Dist(2, 7)` + `NPA_1`")

names(test.form) <- 'formula'

knitr::kable(test.form)

test.form <- "`class` ~ `dip_y` + `Dist(2, 7)` + `NPA_1`"
