---
title: "Aldehyde Deuteration - Model Testing"
output:
  word_document:
    highlight: null
editor_options:
...
sub_model_ordinal <- function(data, out.col = which(colnames(data) == 'class'),
...
# mod.info.ordinal.ordinal description:
# 
...
mod.info.ordinal <- function(model, data, verbose = T, save.acc = F) {
  ...
  test.0 <- MASS::polr(class ~ 1,data = data, Hess = T, control = list(maxit = 100))
  test.1 <- model
...
``` {r Model Search}
data_bin <- data.frame(data.table::fread("United_data_binary_FP_rdkit_3_class.csv"), check.names = F) # Import data
...
# models <- sub_model_ordinal(data = data_bin[uni_similarties, ],
#                                                min = 3,
#                                                max = 3)
  
models <- data.frame(data.table::fread('3_features_models_ordinal_mixed.csv'), check.names = F)[, -1]
 
knitr::kable(models) # present list of models
#write.csv(models, '3_features_models_ordinal_mixed.csv')

test.data <- data_bin[uni_similarties, ]
Test.set <- data_bin[-uni_similarties, ]
...
# While loop to keep trying until the model fits successfully
while (!success) {
  result <- fit_polr_model(test.form, test.data, start)
...
mod.info.ordinal(test, test.data, T, T)
...
mod.info.ordinal(test, Test.set, F, F)
...
### 1. Dist_(P,C)_P Removed
```{r 1.1}


test.form <- data.frame("`class` ~ `NPA_6_SM` + `para_SM`")

names(test.form) <- 'formula'

knitr::kable(test.form)

test.form <- "`class` ~ `NPA_6_SM` + `para_SM`"
...
mod.info.ordinal(test, test.data, T)

ct.plot(class.table, 
        plot.title = 'Dist_(P,C)_P Removed',
        conformation = 'Training Set')
...
mod.info.ordinal(test, Test.set, F)

ct.plot(class.table, 
        plot.title = 'Dist_(P,C)_P Removed',
        conformation = 'Test Set')
...
test.form <- data.frame("`class` ~ `Dist_(P,C)_P` + `para_SM`")

names(test.form) <- 'formula'

knitr::kable(test.form)

test.form <- "`class` ~ `Dist_(P,C)_P` + `para_SM`"
...
test.form <- data.frame("`class` ~ `Dist_(P,C)_P` + `NPA_6_SM`")

names(test.form) <- 'formula'

knitr::kable(test.form)

test.form <- "`class` ~ `Dist_(P,C)_P` + `NPA_6_SM`"
