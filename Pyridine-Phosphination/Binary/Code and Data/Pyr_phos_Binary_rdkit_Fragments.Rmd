---
title: "Pyr Phos - Binary - works against not"
output:
  word_document:
    highlight: null
editor_options:
...
# models <- sub_model_log_McFadden(data = data_bin[c(one, two), ],
#                                                min = 4,
#                                                max = 4)
models <- data.frame(data.table::fread('binary_rdkit_FP_models.csv'))[, -1] 
...
### 5th Place with 5 features (fr_halogen added)
``` {r 5.1.1}
...
knitr::kable(rbind(model.sum.1, model.sum.2, model.sum.3, model.sum.4, 
                   model.sum.5, model.sum.5.1, model.sum.6, model.sum.7, model.sum.8,
