---
title: "Piridine Phosphinat - Best Model Feature Analysis"
output:
  word_document:
    highlight: null
editor_options:
  ...
  if (stratify == T && !is.null(sample.vector) && is.null(folds)) {
    ...
    for (i in 1:length(sort(unique(new_dat$split.assign)))) {
      ...
      probalities[[match(i,sort(unique(new_dat$split.assign)))]] <- cbind(probalities[[match(i,sort(unique(new_dat$split.assign)))]],
                                                                          ...
                                                                          test$flag)
      names(probalities[[match(i,sort(unique(new_dat$split.assign)))]]) <- c('A','B','prediction','flag')
    }
  } else {
    if (folds == nrow(data)) {
    ...
    for (i in 1:folds) {
      ...
      names(probalities[[match(i,1:folds)]]) <- c('A','B','prediction','flag')
    }
  }
  
  probs <- data.frame(do.call(rbind, probalities))
  probs <- probs[order(probs$flag),]
  probs[,1:2] <- round(probs[,1:2],digits = 0)
  pred <- probs[,3]
...
mod.info <- function(model, data, verbose = T, save.acc = F) {
  ...
  pred <- plyr::mapvalues(pred, 
                    from=c("1","2"), 
                    to=c("A","B"), warn_missing = F)
  actual <- data$class
  actual <- plyr::mapvalues(actual, 
                      from=c("1","2"), 
                      to=c("A","B"), warn_missing = F)
  example <- tibble(actual, pred)
  all_vec <- as.factor(LETTERS[1:2])
...
prob.heatmap <- function(model, data, plot.title, conformation) {
  ...
  verif <- data.frame(cbind(as.numeric(as.character(data$class)), pred, r.w, 100 - probs,
                            probs,
                            rep(NA, length(probs))))
  ...
  for (i in 1:dim(verif)[1]) {
    if (verif$r.w[i] == 1) { 
      verif$V6[i] <- "#66a182" # green
    } else {
      verif$V6[i] <- "#d1495b" # red
      }
    }
  
  pro.df <- data.frame(100 - probs, probs)
  pro.df <- rownames_to_column(pro.df)
  pro.df[,1] <- factor(pro.df[,1],levels = pro.df[,1])
  pro.df[,4] <- as.numeric(as.character(data$class))
  colnames(pro.df) <- c('Name',"A",'B','Exp')
...
simi.sampler <- function(data, class, compare.with = 0, plot = F, sample.size = min(summary(as.factor(data$class)))) {
  ...
  simi.table <- dplyr::mutate(simi.table, flag = 1:nrow(simi.table))
...
``` {r Model Search}
data_bin <- data.frame(data.table::fread('United_data_binary_FP_rdkit.csv'), check.names = F) # Import data
...
ct.plot(class.table, 
        ...
        conformation = '5. 5th Place - 5 features')
...
model.sum.5.1 <- data.frame('Training Accuracy' = Acc.print, 'Test Accuracy' = percentage)
row.names(model.sum.5.1) <- '5. 5th Place with 5 features'
```
...
test.form <- data.frame("`class` ~ `fr_Ar_N` + `fr_aryl_methyl` + `fr_benzene` + `fr_halogen`")

names(test.form) <- 'formula'

knitr::kable(test.form)

test.form <- "`class` ~ `fr_Ar_N` + `fr_aryl_methyl` + `fr_benzene` + `fr_halogen`"
...
ct.plot(class.table, 
        plot.title = 'Training Set',
        conformation = '1. Avg_NPA_SM Removed')
...
row.names(model.sum.1) <- '1. Avg_NPA_SM Removed'
```

### 2. fr_Ar_N Removed
```{r 2.1}

test.form <- data.frame("`class` ~ `Avg_NPA_SM` + `fr_aryl_methyl` + `fr_benzene` + `fr_halogen`")

names(test.form) <- 'formula'

knitr::kable(test.form)

test.form <- "`class` ~ `Avg_NPA_SM` + `fr_aryl_methyl` + `fr_benzene` + `fr_halogen`"
...
ct.plot(class.table, 
        plot.title = 'Training Set',
        conformation = '2. fr_Ar_N Removed')
...
row.names(model.sum.2) <- '2. fr_Ar_N Removed'
```

### 3. fr_aryl_methyl Removed
```{r 3.1}

test.form <- data.frame("`class` ~ `Avg_NPA_SM` + `fr_Ar_N` + `fr_benzene` + `fr_halogen`")

names(test.form) <- 'formula'

knitr::kable(test.form)

test.form <- "`class` ~ `Avg_NPA_SM` + `fr_Ar_N` + `fr_benzene` + `fr_halogen`"
...
ct.plot(class.table, 
        plot.title = 'Training Set',
        conformation = '3. fr_aryl_methyl Removed')
...
row.names(model.sum.3) <- '3. fr_aryl_methyl Removed'
```

### 4. fr_benzene Removed
```{r 4.1}

test.form <- data.frame("`class` ~ `Avg_NPA_SM` + `fr_Ar_N` + `fr_aryl_methyl` + `fr_halogen`")

names(test.form) <- 'formula'

knitr::kable(test.form)

test.form <- "`class` ~ `Avg_NPA_SM` + `fr_Ar_N` + `fr_aryl_methyl` + `fr_halogen`"
...
ct.plot(class.table, 
        plot.title = 'Training Set',
        conformation = '4. fr_benzene Removed')
...
row.names(model.sum.4) <- '4. fr_benzene Removed'
```

### 5. fr_halogen Removed
```{r 5.1}

test.form <- data.frame("`class` ~ `Avg_NPA_SM` + `fr_Ar_N` + `fr_aryl_methyl` + `fr_benzene`")

names(test.form) <- 'formula'

knitr::kable(test.form)

test.form <- "`class` ~ `Avg_NPA_SM` + `fr_Ar_N` + `fr_aryl_methyl` + `fr_benzene`"
...
ct.plot(class.table, 
        plot.title = 'Training Set',
        conformation = '5. fr_halogen Removed')
...
row.names(model.sum.5) <- '5. fr_halogen Removed'
```

## Section Results Summary
```{r sum 1}
knitr::kable(rbind(model.sum.5.1, model.sum.1, model.sum.2, model.sum.3, model.sum.4, 
                   model.sum.5))
```