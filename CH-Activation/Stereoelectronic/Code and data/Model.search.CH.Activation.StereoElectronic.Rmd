---
title: "CH Activation - Model Search - Stereoelecronic features"
output:
  word_document:
    highlight: null
editor_options:
...
``` {r Model Search}
data50 <- data.frame(data.table::fread('Data_CH_act_above50.csv')) # Import data
row.names(data50) <- data50[,1]
data50 <- data50[,-1]
data50$class <- as.factor(data50$class)
data50[,c(1:46)] <- as.numeric(as.matrix(data50[,c(1:46)]))
data50 <- plyr::mutate(data50, flag = seq(1,nrow(data50)))

one <- simi.sampler(data50, 1) # Sample from group 1, according to size of smallest group
two <- simi.sampler(data50, 2)
three <- simi.sampler(data50, 3)
four <- simi.sampler(data50, 4)
two_three <- simi.sampler(data50, 2, 3) # Sample from group 2 molecules that are similar to group 1
one_three <- simi.sampler(data50, 1, 3)
four_three <- simi.sampler(data50, 4, 3)


uni_similarties <- c(three,
                      union(two, two_three),
                      union(one, one_three),
                      union(four, four_three))

models <- sub_model_log_McFadden(data = data50[uni_similarties, ],
                                                min = 4,
                                                max = 4)

knitr::kable(models) # present list of models
write.csv(models, '4_features_models_SE.csv')

test.data <- data50[uni_similarties, ]
Test.set <- data50[-uni_similarties, ]

Predictions_data <- data.frame(data.table::fread('Data_CH_act_under50.csv'))
