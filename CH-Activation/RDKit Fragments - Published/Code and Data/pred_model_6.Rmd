---
title: "CH Activation - Fingerprints - Model 3 Predictions"
output:
  word_document:
    highlight: null
editor_options:
...
models <- read.csv('fingerprints_model_4_features.csv')[, -1]
```
...
pred.table <- data.frame(round(cbind(predict(test, Predictions_data), round(predict(test, Predictions_data, 'probs') * 100, 2)), 0))
row.names(pred.table) <- RN
names(pred.table) <- c("Predicted", "1", "2", "3", "4")
knitr::kable(pred.table, caption = 'New Molecules - Prediction of Conditions')
```
